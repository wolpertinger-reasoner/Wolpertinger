 ...
 * an normalized ontology into an answer set program.
 * 
 * 
 * 
 * @see OWLAxiomVisitor
 ...
 *
 */
public class NaiveTranslation implements OWLAxiomVisitor,
	...
	public void translateOntology(OWLAxioms normalizedOntology) {
		...
		for (OWLObjectProperty property : normalizedOntology.m_objectProperties) {
			createExtenstionGuess(property);
			var.reset();
		}
	 ...
	 * @param property
	 */
	private void createExtenstionGuess(OWLObjectProperty property) {
		String owlThing = "thing";
	...
	/**
	 * Provides an "infinite" sequence of variables X,Y,Y1,Y2,...
	 */
	...
	public void visit(OWLObjectSomeValuesFrom objExistential) {
		...
		OWLClass fillerClass = objExistential.getFiller().asOWLClass();
		
		String proprtyName = mapper.getPredicateName(property);
		writer.print(ASP2CoreSymbols.NAF);
		writer.print(" ");
		writer.print(proprtyName);
		writer.print(ASP2CoreSymbols.BRACKET_OPEN);
	...
	public void visit(OWLObjectAllValuesFrom arg0) {
		...
		OWLClass  fillerClass = arg0.getFiller().asOWLClass();
